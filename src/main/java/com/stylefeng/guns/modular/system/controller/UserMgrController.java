...
public class UserMgrController extends BaseController {
    ...
    public String roleAssign(@PathVariable Integer userId, Model model) {
        ...
        SecretUser secretUser = (SecretUser) Db.create(UserMapper.class).selectOneByCon("id", userId);
        model.addAttribute("userId", userId);
        model.addAttribute("userAccount", secretUser.getAccount());
    ...
    public String userEdit(@PathVariable Integer userId, Model model) {
        ...
        SecretUser secretUser = this.userMapper.selectById(userId);
        model.addAttribute(secretUser);
        model.addAttribute("roleName", ConstantFactory.me().getRoleName(secretUser.getRoleId()));
        model.addAttribute("deptName", ConstantFactory.me().getDeptName(secretUser.getDeptId()));
        LogObjectHolder.me().set(secretUser);
    ...
    public Object changePwd(@RequestParam String oldPwd, @RequestParam String newPwd, @RequestParam String rePwd) {
        ...
        String oldMd5 = ShiroKit.md5(oldPwd, secretUser.getSalt());
        if (secretUser.getPassword().equals(oldMd5)) {
            String newMd5 = ShiroKit.md5(newPwd, secretUser.getSalt());
            secretUser.setPassword(newMd5);
            secretUser.updateById();
            return SUCCESS_TIP;
        } else {
    ...
    public Tip add(@Valid UserDto user, BindingResult result) {
            ...
            // 判断账号是否重复
            SecretUser theSecretUser = managerDao.getByAccount(user.getAccount());
            if (theSecretUser != null) {
        ...
        user.setUserStatus(ManagerStatus.OK.getCode());
    ...
    @ResponseBody
    public SecretUser view(@PathVariable Integer userId) {
        if (ToolUtil.isEmpty(userId)) {
            throw new BussinessException(BizExceptionEnum.REQUEST_NULL);
        }
    ...
    public Tip reset(@RequestParam Integer userId) {
        ...
        SecretUser secretUser = this.userMapper.selectById(userId);
        secretUser.setSalt(ShiroKit.getRandomSalt(5));
        secretUser.setPassword(ShiroKit.md5(Const.DEFAULT_PWD, secretUser.getSalt()));
        this.userMapper.updateById(secretUser);
        return SUCCESS_TIP;
    }
